...
class HHAVWGCN(nn.Module):
    def __init__(self, dim_in, dim_out, PoicareBall,cheb_k,embed_dim,dropout,c0,c1):
        super(HHAVWGCN, self).__init__()
        self.PoicareBall=PoicareBall
        self.c0=c0
        self.layer1 = HGCNConv(PoicareBall, dim_in, dim_out, cheb_k,embed_dim,c0, c1,
                                   dropout=dropout)
    def forward(self, x, node_embeddings):
        ...
        x_gconv=self.layer1.forward(x_hyp,node_embeddings)
        return x_gconv
...
class HHAGCRNCell(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim,order,dropout):
        ...
        self.gate = HHAVWGCN(dim_in+self.hidden_dim+self.order, dim_out, self.PoicareBall, cheb_k,embed_dim,dropout,self.hypebolic_c[0],self.hypebolic_c[1])
        self.update = HHAVWGCN(dim_in+self.hidden_dim+self.order, dim_out, self.PoicareBall, cheb_k,embed_dim,dropout,self.hypebolic_c[0],self.hypebolic_c[1])
    ...
    def forward(self, x, state, c, node_embeddings,t):
        ...
        g = torch.sigmoid(self.toTangentX(self.gate(input_and_state, node_embeddings),self.hypebolic_c[0]))
        hc = torch.tanh(self.toTangentX(self.update(input_and_state, node_embeddings),self.hypebolic_c[0]))
