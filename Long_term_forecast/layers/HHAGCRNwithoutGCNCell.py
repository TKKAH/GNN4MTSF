...
class HHAVWGCN(nn.Module):
    def __init__(self, dim_in, dim_out, cheb_k, embed_dim,device):
        ...
        self.weights_pool = nn.Parameter(torch.ones(cheb_k, dim_in, dim_out))
        self.bias_pool = nn.Parameter(torch.zeros(dim_out))
    ...
    def forward(self, x, node_embeddings,adj):
        ...
        #     x_g=x_g[:,1,...]
        # x_gconv = torch.einsum('bnki,kio->bno', x_g, self.weights_pool) + self.bias_pool     #b, N, dim_out
        # return x_gconv
        #withoutadaptivegraph:
        # node_num = adj.shape[0]
        # adj_new = torch.ones((node_num, node_num))
        # if x.shape[1]==node_num:
        #     supports = adj_new
        #     support_set = [torch.eye(node_num).to(supports.device), supports]
        # elif x.shape[1]==3*node_num:
        #     supports = self.construct_adj(adj_new,3)
        #     supports=torch.tensor(supports).to(torch.float32).to(self.device)
        #     support_set = [torch.eye(3*node_num).to(supports.device), supports]
        # for k in range(2, self.cheb_k):
        #     support_set.append(torch.matmul(2 * supports, support_set[-1]) - support_set[-2])
