...
from utils.laplacian_matrix import scaled_Laplacian,cheb_polynomial
...
class ASTGCN_block(nn.Module):
    ...
    def __init__(self,in_channels,device,args,cheb_polynomials):
        super(ASTGCN_block, self).__init__()
        self.TAt = Temporal_Attention_layer(device, in_channels, args.num_nodes, args.seq_len)
        self.SAt = Spatial_Attention_layer(device, in_channels, args.num_nodes, args.seq_len)
        self.cheb_conv_SAt = cheb_conv_withSAt(args.ASTGCN_cheb_k, cheb_polynomials, in_channels, args.ASTGCN_nb_chev_out_dim)
        self.time_conv = nn.Conv2d(args.ASTGCN_nb_chev_out_dim, args.ASTGCN_nb_time_out_dim, kernel_size=(1, 3), stride=(1, 1), padding=(0, 1))
        self.residual_conv = nn.Conv2d(in_channels, args.ASTGCN_nb_time_out_dim, kernel_size=(1, 1), stride=(1, 1))
        self.ln = nn.LayerNorm(args.ASTGCN_nb_time_out_dim)  #需要将channel放到最后一个维度上
...
class Model(nn.Module):
    ...
    def __init__(self, args, adj_mx, device):
        ...
        self.final_conv = nn.Conv2d(args.seq_len, args.pred_len, kernel_size=(1, args.ASTGCN_nb_time_out_dim))
