...
from layers.STWA_attention import TemporalAttention, SpatialAttention
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.start_fc = nn.Linear(in_features=config.input_dim, out_features=self.channels)
        self.memory_size = config.memory_size
        self.pred_len=config.pred_len

        self.layers = nn.ModuleList(
            [
                Layer(device='cuda:0', input_dim=config.channels, dynamic=config.dynamic, num_nodes=config.num_nodes, cuts=12,
                      cut_size=6, no_proxies=2, memory_size=config.memory_size),
                Layer(device='cuda:0', input_dim=config.channels, dynamic=config.dynamic, num_nodes=config.num_nodes, cuts=3,
                      cut_size=4, no_proxies=2, memory_size=config.memory_size),
                Layer(device='cuda:0', input_dim=config.channels, dynamic=config.dynamic, num_nodes=config.num_nodes, cuts=1,
                      cut_size=3, no_proxies=2, memory_size=config.memory_size),
            ])

        self.skip_layers = nn.ModuleList([
            nn.Linear(in_features=12 * config.channels, out_features=256),
            nn.Linear(in_features=3 * config.channels, out_features=256),
            nn.Linear(in_features=1 * config.channels, out_features=256),
        ])

        self.projections = nn.Sequential(*[
            nn.Linear(256, 512),
            nn.ReLU(),
            nn.Linear(512, config.pred_len)])

        if self.dynamic:
            self.mu_estimator = nn.Sequential(*[
                nn.Linear(config.input_dim * self.seq_len, 32),
                nn.Tanh(),
                nn.Linear(32, 32),
                nn.Tanh(),
                nn.Linear(32, config.memory_size)
            ])
    ...
    def forecast(self, x):
        ...
        output=self.projections(x).transpose(2, 1).squeeze()
